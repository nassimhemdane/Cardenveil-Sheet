...
function render() {
  ...
  rectoPage.className = "sheet-page sheet-page-main";
  ...
  for (const page of state.template.pages.filter((item) => item.id !== "recto")) {
    ...
    pageEl.className = "sheet-page sheet-page-flow";
...
async function exportPackage() {
  setStatus("Préparation de l'archive...");
  await saveCharacter();
  ...
  a.href = `/api/export-package?id=${encodeURIComponent(state.character.id)}&t=${Date.now()}`;
  a.download = `${state.character.id || "personnage"}.zip`;
  a.click();
  setStatus("Archive exportée");
}
